...
export const TenantIdHeaderSchema = z
  .object({
    "x-jackline-tenant-id": z.uuid().optional().openapi({
      param: {
        name: "x-jackline-tenant-id",
        in: "header",
        required: false,
      },
      example: "00000000-0000-4000-8000-000000000000",
      description:
        "Required for session auth (send as X-Jackline-Tenant-Id). Optional for OAuth2 Bearer tokens (must match token tenant if set).",
    }),
  })
