...
import type { PublicServer, PublicTool, ToolStatus } from "@mesh/shared";
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        eyebrow="Catalog"
        title="Tools"
        description="Named tools on upstream servers. Only active tools can pass policy."
        actions={
...
function CreateToolForm({
  ...
  async function onSubmit(e: FormEvent) {
    ...
    setBusy(true);
    try {
      await meshApi.createTool(tenantId, { name, serverId, status });
      await onCreated();
    } catch (err) {
